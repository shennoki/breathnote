...
export const ALL_CATEGORIES = (async () => {
  const categories: AllCategoriesType = await fetch(`${process.env.API_ENDPOINT}/categories`, KEY)
    .then((res) => res.json())
    .catch((err) => fetchFailed(err))
  const limit = 10
  ...
  for (let i = 1; i <= roopCount; i++) {
    const fetchData = await fetch(`${process.env.API_ENDPOINT}/categories?offset=${limit * i}&limit=${limit}`, KEY)
      .then((res) => res.json())
      .catch((err) => fetchFailed(err))
...
export const ALL_TAGS = (async () => {
  const tags: AllCategoriesType = await fetch(`${process.env.API_ENDPOINT}/tags`, KEY)
    .then((res) => res.json())
    .catch((err) => fetchFailed(err))
  const limit = 10
  ...
  for (let i = 1; i <= roopCount; i++) {
    const fetchData = await fetch(`${process.env.API_ENDPOINT}/tags?offset=${limit * i}&limit=${limit}`, KEY)
      .then((res) => res.json())
      .catch((err) => fetchFailed(err))
